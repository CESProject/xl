            ...
            complete:(void (^)(BOOL successed, NSDictionary *result))complete
{
    ...
    NSInteger tim =[[NSDate date] minutesAfterDate:[[NSUserDefaults standardUserDefaults] objectForKey:@"time"]];
    if (tim>55)
    {
        self.url = url;
        self.httpMethod = httpMethod;
        self.params = params;
        self.lastCompleteBlock = complete;
        
        NSMutableURLRequest *request = [self requestWithUrl:DEF_LOGIN httpMethod:@"POST" useCache:NO params:@{@"username":[[NSUserDefaults standardUserDefaults] objectForKey:@"username"],@"password":[[NSUserDefaults standardUserDefaults] objectForKey:@"userpass"]}];
        void (^requestSuccessBlock)(AFHTTPRequestOperation *operation, id responseObject) = ^(AFHTTPRequestOperation *operation, id responseObject) {
            [MBProgressHUD hideHUD];
             NSDictionary *result = [self dictionaryWithData:responseObject];
            ...
            [self requestToUrl:self.url httpMethod:self.httpMethod useCache:NO params:self.params complete:self.lastCompleteBlock];
    ...
    }else
    {
        ...
        void (^requestSuccessBlock)(AFHTTPRequestOperation *operation, id responseObject) = ^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if ([dic[@"msg"] isEqualToString:@"sessionInvalid"]&&[dic[@"code"] isEqualToString:@"11111"]) {
                [self requestToUrl:DEF_LOGIN httpMethod:@"POST" useCache:NO params:@{@"username":[[NSUserDefaults standardUserDefaults] objectForKey:@"username"],@"password":[[NSUserDefaults standardUserDefaults] objectForKey:@"userpass"]} complete:^(BOOL successed, NSDictionary *result) {
                    User *user = [[User alloc] init];
                    user.userId = result[@"id"];
                    user.loginName = result[@"loginName"];
                    user.imageDiyBgVo = result[@"imageDiyBgVo"];
