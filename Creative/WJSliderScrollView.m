...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, self.sliderView.frame.size.height, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) + self.sliderView.frame.size.height  + viewSize.height + 350)];
...
- (void)commonInitWithItemArray:(NSArray<UIView *> *)itemArray contentArray:(NSArray<UIView *>*)contentArray {
    ...
    viewSize.height = CGRectGetWidth(self.bounds)*itemArray.count;
...
- (void)WJSliderViewDidIndex:(NSInteger)index {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(loadWJSliderViewDidIndex:)]) {
        [_delegate loadWJSliderViewDidIndex:index];
    }
}
